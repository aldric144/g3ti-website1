...
async def submit_classified_request(data: ClassifiedAccessRequest, background_tasks: BackgroundTasks, request: Request):
    ...
    # Generate reference ID
    reference_id = f"G3TI-CLS-{datetime.utcnow().strftime('%Y%m%d')}-{secrets.token_hex(3).upper()[:5]}"
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (reference_id, data.full_name, data.email, data.agency, data.clearance_level,
                  data.purpose, data.supervisor_name, data.supervisor_email, risk_score,
                  ip_address, user_agent, geo_str))
            await db.commit()
    except Exception as e:
    ...
    await log_action(
        ...
        details=f"ref={reference_id},clearance={data.clearance_level},agency={data.agency}",
        risk_score=risk_score
    )
    ...
    return {
        "success": True,
        "reference_id": reference_id,
        "message": "Your classified access request has been received by G3TI Security Division. Review time: 24-48 hours."
    }
...
async def get_classified_stats(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cursor = await db.execute("SELECT status, COUNT(*) as count FROM classified_access_requests GROUP BY status")
        status_counts = {row[0]: row[1] for row in await cursor.fetchall()}
        
        # Total counts by clearance level
        cursor = await db.execute("SELECT clearance_level, COUNT(*) as count FROM classified_access_requests GROUP BY clearance_level")
        ...
        cursor = await db.execute("""
            SELECT COUNT(*) FROM classified_access_requests 
...
async def get_classified_request_detail(request_id: int, credentials: HTTPAuthorizationCredentials = Depends(security)):
    """Get detailed view of a single classified request"""
    if not credentials or not verify_admin(credentials.credentials):
        raise HTTPException(status_code=401, detail="Admin authentication required")
